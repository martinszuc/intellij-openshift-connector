...
//    @Test
    ...
    public void pasteLoginCommandTest() {
        ...
        String loginCommand = "oc login -u testUser -p testPassword https://api.sample-url.com";
        StringSelection stringSelection = new StringSelection(loginCommand);
        ...
        assertEquals("testUser", clusterLoginDialog.getUsername());
        assertEquals("testPassword", clusterLoginDialog.getPassword());

        assertEquals("", clusterLoginDialog.getToken());

        LOGGER.info("Closing cluster login dialog");
        clusterLoginDialog.close();
        LOGGER.info("pasteLoginCommandTest: End");
    }
